...
class LongTermMemory {
    constructor() {
        ...
        this.embeddings = new Map(); // Future: move to vector DB
    }
    /**
     ...
     */
    async updateAccessMetadata(entryId) {
        const entry = this.entries.get(entryId);
        if (entry) {
            entry.accessCount += 1;
     ...
     */
    matchesVectorFilters(entry, filters) {
        if (!filters)
            return true;
    ...
    async consolidateGroup(group) {
        ...
        const latestTimestamp = group.reduce((latest, e) => e.timestamp > latest ? e.timestamp : latest, group[0].timestamp);
        return {
